...
with src(recipient_phone, event_type, message_body, status, sent_at, fonnte_id) as (
  values
...
select
  (select id from profiles where regexp_replace(coalesce(whatsapp_number,''),'[^0-9]','','g') = src.recipient_phone limit 1),
  src.recipient_phone, src.event_type, src.message_body, src.status, src.sent_at::timestamptz, src.fonnte_id
from src
