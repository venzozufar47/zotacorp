...
exports.parseIndoAmount = parseIndoAmount;
exports.parseIndoDate = parseIndoDate;
exports.verifyBalance = verifyBalance;
exports.validateZeroSum = validateZeroSum;
exports.inferPeriodFromDates = inferPeriodFromDates;
 ...
 */
function parseIndoDate(raw, defaultYear) {
    if (!raw)
        return null;
    const s = raw.trim();
    if (!s)
 ...
 */
function verifyBalance(openingBalance, closingBalance, transactions) {
 ...
 */
function validateZeroSum(transactions) {
    ...
    for (let i = 1; i < chronological.length; i++) {
        ...
        if (typeof prev.runningBalance !== "number" ||
        ...
        if (diff > 1) {
            ...
            if (mismatchCount <= 3) {
                warnings.push(`Saldo tidak konsisten di ${cur.date} "${cur.description.slice(0, 60)}${cur.description.length > 60 ? "…" : ""}": seharusnya ${formatRp(Math.round(expected))}, ter-parse ${formatRp(cur.runningBalance)}. Kemungkinan angka yang dibaca salah.`);
            }
        }
    }
    if (mismatchCount > 3) {
        warnings.push(`…dan ${mismatchCount - 3} baris lain dengan saldo tidak konsisten. Cek manual di tabel review.`);
    }
    return warnings;
}
/**
 ...
 * valid transactions.
 */
function inferPeriodFromDates(transactions) {
