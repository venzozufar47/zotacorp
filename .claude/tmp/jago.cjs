...
exports.parseJagoStatement = parseJagoStatement;
;
const shared_cjs_1 = require("C:/zotacorp/.claude/tmp/shared.cjs");
const HEADERS = [
    "date & time",
    "source/destination",
    "transaction details",
...
async function parseJagoStatement(buffer, 
...
_password) {
    ...
    for (let i = headerIdx + 1; i < rows.length; i++) {
        ...
        const sourceDestination = cols.source !== -1 ? cellStr(row[cols.source]) : "";
        const transactionDetails = cols.details !== -1 ? stripIdHash(cellStr(row[cols.details])) : "";
        const notes = cols.notes !== -1 ? cellStr(row[cols.notes]) : "";
        const balance = parseUnsignedAmount(row[cols.balance]);
        const descFields = [sourceDestination, transactionDetails, notes].filter(Boolean);
    ...
    if (transactions.length === 0) {
        warnings.push("Tidak ada transaksi yang ter-parse dari file. Pastikan formatnya sesuai export Jago (Date & Time, Source/Destination, Transaction Details, Notes, Amount, Balance).");
    }
    const period = (0, shared_cjs_1.inferPeriodFromDates)(transactions);
    return {
...
function detectHeader(rows) {
    ...
    for (let i = 0; i < maxScan; i++) {
        ...
        if (positions["date & time"] !== undefined &&
...
function parseDateTime(raw) {
    ...
    if (tokens.length < 3) {
        const isoOnly = (0, shared_cjs_1.parseIndoDate)(trimmed);
        return { date: isoOnly };
    }
    const dateStr = tokens.slice(0, 3).join(" ");
    const timeStr = tokens.slice(3).join(" ");
    const isoDate = (0, shared_cjs_1.parseIndoDate)(dateStr);
    if (!isoDate)
        return { date: null };
...
function parseSignedAmount(raw) {
    ...
    let lastSigned = null;
    let m;
    while ((m = signedRe.exec(trimmed)) !== null)
        lastSigned = m;
    if (lastSigned) {
        const sign = lastSigned[1] === "-" ? "-" : "+";
        const body = lastSigned[2];
        const n = (0, shared_cjs_1.parseIndoAmount)(normalizeJagoNumber(body));
        return { sign, value: Math.max(0, Math.round(Math.abs(n))) };
    }
    // Tidak ada sign — ambil token whitespace-terpisah terakhir.
    const tokens = trimmed.split(/\s+/).filter(Boolean);
    const last = tokens[tokens.length - 1] ?? "";
    const n = (0, shared_cjs_1.parseIndoAmount)(normalizeJagoNumber(last));
    return {
        sign: n < 0 ? "-" : "+",
 ...
 * jadi `15.390.687`.
 */
function parseUnsignedAmount(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return null;
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
        ...
        if (!/\d/.test(tok))
            continue;
        const n = (0, shared_cjs_1.parseIndoAmount)(normalizeJagoNumber(tok));
 ...
 *   lainnya       → apa adanya
 */
function normalizeJagoNumber(s) {
